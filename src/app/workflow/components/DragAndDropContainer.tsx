...
import ReactFlow, { addEdge, Controls, Background, useReactFlow, Node, Edge, XYPosition, Position, Connection, ReactFlowProvider } from 'reactflow';
...
import { Filter, Sort, Conditional, GroupBy, Statistical, NodeData, Arithmetic, Scaling, CustomNode, Condition, Merge } from '../../types/workflowTypes';
...
const DragAndDropContainer: React.FC<DragAndDropContainerProps> = ({
  ...
  const [newNodeData, setNewNodeData] = useState<{ id: string; position: XYPosition; icon: keyof typeof Icons | keyof typeof FaIcons } | null>(null);
  ...
  const handleStartModalOk = useCallback((values: any, isMergeSelected: boolean) => {
    if (newNodeData) {
      const tableName = isMergeSelected
        ? `${values.table1} & ${values.table2}`
        : values.table1Single;
      ...
      setSidebarItems((items) =>
        items.map((item) =>
          item.id !== 'start' ? { ...item, enabled: true } : item
        )
      );
    }
  }, [newNodeData, setNodes, setSidebarItems]);



  const handleOutputModalOk = useCallback((values: any) => {
    if (newNodeData) {
      const newNode: Node = {
        id: newNodeData.id,
        data: {
          ...
          label: createNodeLabel(values.outputName, 'Output Node', undefined, newNodeData.id, false, true),
        },
        position: newNodeData.position,
        draggable: true,
        sourcePosition: Position.Right,
  ...
  const handlePivotTableModalOk = useCallback((values: any) => {
    if (newNodeData && selectedTable) {
      const newNode: Node = {
        id: newNodeData.id,
        data: {
          table: selectedTable,
          ...
          label: createNodeLabel(selectedTable, 'Pivot Node', {
            type: 'pivotTable',
            pivotTable: {
              pivotColumns: values.pivotColumns,
              functionCheckboxes: values.functionCheckboxes,
  ...
  }, [newNodeData, selectedTable, setNodes]);

  const handleGroupByModalOk = useCallback((values: any) => {
    if (newNodeData && selectedTable) {
      const newNode: Node = {
        id: newNodeData.id,
        data: {
          ...
          label: createNodeLabel(selectedTable, 'Group By Node', {
            groupByColumns: values.groupbyColumn.index,
            targetColumns: values.groupbyColumn.value,
            functionCheckboxes: values.functionCheckboxes,
          }, newNodeData.id),
  ...
  const handleConditionalModalOk = useCallback(
    (values: any) => {
      if (newNodeData && selectedTable) {
        const basePosition = newNodeData.position;
        const offsetX = 250;
        const offsetY = 100;
        ...
        const conditionsData = values.conditions.map((condition: Condition, index: number) => {
          const conditionTypeLabel =
            values.conditionType === 'if/else'
              ...
              : values.conditionType === 'else/if'
                ? index === 0 ? 'If' : index === 1 ? 'Else If' : 'Else'
                : 'If';

          const mappedSubConditions: Condition[] = condition.subConditions?.map(subCondition => ({
            id: subCondition.id,
          ...
          const mappedOutsideConditions: Condition[] = condition.outsideConditions?.map(outsideCondition => ({
        ...
        const mainNode: Node = {
          ...
          data: {
            ...
            label: createNodeLabel(selectedTable, 'Conditional Node', {
        ...
        setNodes((nds) => [...nds, mainNode, ...newNodes]);
        setEdges((eds) =>
          eds.concat(
            newNodes.map((node: Node, index: number) => ({
              id: `edge-${newNodeData.id}-${node.id}`,
    ...
    [newNodeData, selectedTable, setNodes, setEdges]
  );

  const handleSortModalOk = useCallback((values: any) => {
    if (newNodeData && selectedTable) {
      const newNode: Node = {
        id: newNodeData.id,
        data: {
          ...
          label: createNodeLabel(selectedTable, 'Sort Node', {
            column: values.column,
            sortType: values.sortType,
          }, newNodeData.id),
        },
  ...
  const handleFilterModalOk = useCallback((values: any) => {
    if (newNodeData && selectedTable) {
      let formattedValue = values.value;

      if (confirmedDataType === 'date' && values.value instanceof Array) {
        const [startDate, endDate] = values.value;
        formattedValue = `Start: ${startDate.format('YYYY-MM-DD')}, End: ${endDate.format('YYYY-MM-DD')}`;
      }
      ...
      const newNode: Node = {
        ...
        data: {
          ...
          label: createNodeLabel(selectedTable, 'Filter Node', {
            column: values.column,
            operator: values.operator,
            value: formattedValue,
          }, newNodeData.id),
  ...
  }, [newNodeData, selectedTable, setNodes, confirmedDataType]);


  const handleStatisticalModalOk = useCallback((values: any) => {
    if (newNodeData && selectedTable) {
      const newNode: Node = {
        id: newNodeData.id,
        data: {
          ...
          label: createNodeLabel(selectedTable, 'Statistical Node', {
            column: values.column,
            statisticalFunction: values.statisticalfunction,
          }, newNodeData.id),
        },
  ...
  const handleScalingModalOk = useCallback((values: any) => {
    if (newNodeData && selectedTable) {
      const newNode: Node = {
        id: newNodeData.id,
        data: {
          ...
          label: createNodeLabel(selectedTable, 'Scaling Node', {
            column: values.column,
            scalingFunction: values.scalingFunction,
            minValue: values.minValue,
            maxValue: values.maxValue,
  ...
  const handleArithmeticModalOk = useCallback((values: any) => {
    if (newNodeData && selectedTable) {
      ...
      const newNode: Node = {
        ...
        data: {
          ...
          label: createNodeLabel(selectedTable, 'Arithmetic Node', {
            sourceColumn: values.sourceColumns,
            targetvalue: values.targetvalue,
            operation: labelContent,
          }, newNodeData.id),
    ...
    if (itemData.title === 'Filter') {
      ...
      const newNode: Node = {
        ...
        data: {
          ...
          type: 'node',
          label: createNodeLabel(itemData.title, 'Node'),
        },
        position,
        draggable: true,
        targetPosition: Position.Left,
  ...
  }, [nodes, screenToFlowPosition, showModal, showFilterModal, showSortModal, showPivotTableModal, showConditionalModal, showGroupByModal, showStatisticalModal, showScalingModal, showArithmeticModal, showStartModal, setNodes]);
